...
export const getCommentByUser = (userId: string) =>
...
export const updateCommentById = (id: string, values: Record<string, any>) =>
  CommentModel.findByIdAndUpdate(id, values).then((comment) =>
    comment?.toObject()
  )
...
export const deleteCommentByPost = (postId: string) =>
  CommentModel.deleteMany({ post: postId })

export const deleteCommentByUser = (userId: string) =>
